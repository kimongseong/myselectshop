...
public class ProductService {
    ...
    public ProductResponseDto updateProduct(Long id, ProductMypriceRequestDto requestDto) {
        ...
        if (myprice < MIN_MY_PRICE) {
            throw new IllegalArgumentException("유효하지 않는 관심 가격 입니다 최소 " + MIN_MY_PRICE + "원 이상으로 설정해 주세요.");
        }
    ...
    public void updateBySearch(Long id, ItemDto itemDto) {
        Product product = productRepository.findById(id).orElseThrow(() -> new NullPointerException("해당 상품이 존재하지않습니다."));
    ...
    public void addFolder(Long productId, Long folderId, User user) {
        ...
        Folder folder = folderRepository.findById(folderId).orElseThrow(() -> new RuntimeException("해당 폴더가 존재하지 않습니다"));
